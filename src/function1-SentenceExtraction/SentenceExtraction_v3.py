...
section_list = ['HISTORY OF PRESENT ILLNESS', 'PHYSICAL EXAMINATION', 'LABORATORY DATA', 'ASSESSMENT AND PLAN',
                'NARRATIVE']


def extract_sentences(noteId_list, data_path='.', sections=section_list):
    res_list = []

    for notesId in noteId_list:
        try:
            f = open(data_path + '/' + notesId + ".txt", 'r')
            notesText = f.read()
        except FileNotFoundError:
            print(f'Error: no such file named {data_path}/{notesId}.txt')
        else:
            f.close()
