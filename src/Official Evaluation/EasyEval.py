    ...
    def _get_annotations(self):
        ...
        with open(self.path) as annotation_file:
            ...
            for key in attribute_mapper.keys():
                annotations['attributes']['combined_' + key] = Attribute('combined_' + key,
...
# 展开嵌套列表
def pred2series(pred_result):
    unfolder = []
    [[unfolder.append(pred_result[i][j]) for j in range(len(pred_result[i]))] for i in range(len(pred_result))]
    return pd.Series(unfolder)


# 加入fileID和setoff
def add_metadata(test_df, pred_series):
    test_df["Predict"] = pred_series
    test_df["FileId"] = test_df.SentenceId.apply(lambda x: x[:6])
    return test_df.loc[pred_series != "O", ["FileId", "Start", "End", "Word", "Predict"]]


# 单个.ann文件写入
def write_single_ann(file_id, word_list):
    out_str = ""
    for i in range(len(word_list)):
        ...
        out_str = out_str + \
                  "T{id}\tDisposition {start} {end}\t{word}\n".format(id=i + 1, start=start, end=end, word=word)
    with open("./.sys_out/" + file_id + ".ann", "w") as file:
        # print("Write to file {}.ann".format(file_id))
        # print(out_str)
        file.write(out_str)


# 输出所有.ann文件
def write2ann(df, auto_merge=False):
    if not os.path.exists("./.sys_out"):
        os.mkdir("./.sys_out")

    word_list = []
    pre_file_id = df.iloc[0].FileId
    pre_word = {"index": 0}
    ...
    for i, row in df.iterrows():
        ...
        if file_id != pre_file_id:
            # 写入文件
            write_single_ann(pre_file_id, word_list)
            # 清空队列，并把当前实体放入队列
            word_list = [{"Start": start, "End": end, "Word": word, "index": i}]

        # 说明不是第一个实体
        else:
            # word_list.append({"Start":start, "End": end, "Word": word, "index": i})
            # 如果是"I"
            if predict == "I" \
                    or (auto_merge and i == pre_word["index"] + 1):  # 自动merge一些"O"为"I":连在一块肯定是一个词
                # 把当前实体附加到前驱实体后面变成组合词(实体)
                pre_word["Word"] = pre_word["Word"] + " " + word
                pre_word["End"] = end
            # 如果是"B"
            else:
                # 只需把当前实体放入队列
        ...
        # 无论该实体是否是第一个实体，都要重置前驱实体
        pre_word = word_list[-1]
        # 同时重置文件名索引
        pre_file_id = file_id

    write_single_ann(pre_file_id, word_list)  # 导出最后一个文件


class Track1Subtask1Evaluation:
    def __init__(self):
        if not os.path.exists("./.gold.zip"):
            print("I need file '.gold.zip'!")
            raise FileExistsError
        # create_environment
        unzip("./.gold.zip", "./")
        print("Create file 'SetOffs.csv'\nCreate folder '.gold'")
        self.eval_df = pd.read_csv("./SetOffs.csv")

    def eval(self, y_pred, zipfolder=False):
        if len(y_pred) == 5325:
            y_pred = pred2series(y_pred)
        elif len(y_pred) != 46696:
            print("The Size of y_pred can not fit the dev set!")

        self.eval_df["Predict"] = y_pred
        write2ann(self.eval_df[self.eval_df.Predict != "O"])

        if zipfolder:
            make_zip("./.sys_out", "sys_out.zip")

        main("./.gold", "./.sys_out", False)

    def eval_from_folder(self, path):
        main("./.gold", path, False)
